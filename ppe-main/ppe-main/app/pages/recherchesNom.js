...
export default function RecherchesNom() {
  ...
  return (
    <>
    <Head>
      <title>Recherches Nom - Local.i</title>
    </Head>
          ...
          <ul className="bg-white shadow-lg w-full mt-2 rounded-b-lg overflow-hidden">
            {predictions.map((prediction) => (
              <li
                ...
                onClick={() => handleClick(prediction.commercantId)}
              >
                <Link href={`/commercants/${prediction.commercantId}`}>
                  {prediction.nom}
                </Link>
              </li>
            ))}
